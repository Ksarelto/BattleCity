...
const TILES: TileId[] = ['empty', 'brick', 'steel', 'water', 'ice', 'bush'];
const STORAGE_KEY = 'battlecity-custom-levels';

export function EditorScreen() {
  const [level, setLevel] = useState<LevelData>(createBlankLevel);
  const [selectedTile, setSelectedTile] = useState<TileId>('brick');
  ...
  const saveLevel = () => {
    const stored = JSON.parse(localStorage.getItem(STORAGE_KEY) ?? '[]') as LevelData[];
