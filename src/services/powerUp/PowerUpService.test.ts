...
import { collectPowerUp, spawnPowerUp, updateTimerEffect } from '@/services/powerUp/PowerUpService';
...
describe('powerUpSystem', () => {
  ...
  it('shovel fortifies base walls to steel', () => {
    const world = makeWorld();
    const { col, row } = world.basePosition;
    world.grid[row - 1]![col] = { id: 'brick', brick: { tl: true, tr: true, bl: true, br: true } };
    collectPowerUp(world, world.players[0]!, 'shovel');
    expect(world.grid[row - 1]![col]!.id).toBe('steel');
    expect(world.fortifiedUntil).toBeGreaterThan(world.tick);
    expect(world.originalBaseWalls.length).toBeGreaterThan(0);
  });
});
